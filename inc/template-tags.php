...
if ( ! function_exists( 'chainx_posted_on' ) ) :
	...
	function chainx_posted_on() {
		...
		$posted_on = sprintf(
			...
			esc_html_x( 'Posted on %s', 'post date', 'chainx' ),
...
if ( ! function_exists( 'chainx_posted_by' ) ) :
	...
	function chainx_posted_by() {
		$byline = sprintf(
			...
			esc_html_x( 'by %s', 'post author', 'chainx' ),
	...
	function chainx_entry_footer() {
		...
		if ( 'post' === get_post_type() ) {
			...
			if ( $categories_list ) {
				...
				printf( '<span class="cat-links">' . esc_html__( 'Posted in %1$s', 'chainx' ) . '</span>', $categories_list ); // WPCS: XSS OK.
			}
