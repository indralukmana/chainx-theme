...
if ( ! function_exists( 'chainx_setup' ) ) :
	...
	function chainx_setup() {
		...
		add_image_size( 'chainx-index-image', 800, 450, true);
 ...
 */
function chainx_content_image_sizes_attr( $sizes, $size ) {
	$width = $size[0];
...
add_filter( 'wp_calculate_image_sizes', 'chainx_content_image_sizes_attr', 10, 2 );
 ...
 */
function chainx_header_image_tag( $html, $header, $attr ) {
	if ( isset( $attr['sizes'] ) ) {
		$html = str_replace( $attr['sizes'], '100vw', $html );
	}
	return $html;
}
add_filter( 'get_header_image_tag', 'chainx_header_image_tag', 10, 3 );
 ...
 */
function chainx_post_thumbnail_sizes_attr( $attr, $attachment, $size ) {
...
add_filter( 'wp_get_attachment_image_attributes', 'chainx_post_thumbnail_sizes_attr', 10, 3 );
...
function chainx_widgets_init() {
	...
	register_sidebar( array(
		'name'          => esc_html__( 'Page Widget', 'chainx' ),
		'id'            => 'page-1',
		'description'   => esc_html__( 'Add widgets here.', 'chainx' ),
		'before_widget' => '<section id="%1$s" class="widget %2$s">',
		'after_widget'  => '</section>',
